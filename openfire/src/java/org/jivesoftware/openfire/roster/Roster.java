 ...
 * </p>
 * <p>
 * Rosters are similar to buddy groups in popular IM clients. The Roster class
 * is a representation of the roster data.
 * <p/>
 * 
 * <p>
 * Updates to this roster is effectively a change to the user's roster. To
 * reflect this, the changes to this class will automatically update the
 * persistently stored roster, as well as send out update announcements to all
...
public class Roster implements Cacheable, Externalizable {
	...
	/**
	 * Contacts with subscription FROM that only exist due to shared groups key:
	 * jabberid string; value: groups why the implicit roster item exists (aka
	...
	/**
	 * Create a roster for the given user, pulling the existing roster items out
	 * of the backend storage provider. The roster will also include items that
	 * belong to the user's shared groups.
	 * <p>
	 * 
	 * RosterItems that ONLY belong to shared groups won't be persistent unless
	 * the user explicitly subscribes to the contact's presence, renames the
	 * contact in his roster or adds the item to a personal group.
	 * <p>
	 * 
	 ...
	 * 
	 * @param username
	 *            The username of the user that owns this roster
	 */
	Roster(String username) {
		...
		Collection<Group> sharedGroups = rosterManager
				.getSharedGroups(username);
		...
		while (items.hasNext()) {
			...
			// Check if the item (i.e. contact) belongs to a shared group of the
			// user. Add the
			// shared group (if any) to this item
			for (Group group : sharedGroups) {
				if (group.isUser(item.getJid())) {
					// TODO Group name conflicts are not being considered (do we
					// need this?)
		...
		for (Map.Entry<JID, List<Group>> entry : sharedUsers.entrySet()) {
			...
			try {
				...
				String nickname = "";
				RosterItem item = new RosterItem(jid, RosterItem.SUB_TO,
						RosterItem.ASK_NONE, RosterItem.RECV_NONE, nickname,
						null);
				...
				// Set subscription type to BOTH if the roster user belongs to a
				// shared group
				// that is mutually visible with a shared group of the new
				// roster item
				if (rosterManager.hasMutualVisibility(username, sharedGroups,
						jid, itemGroups)) {
					item.setSubStatus(RosterItem.SUB_BOTH);
				} else {
					// Set subscription type to FROM if the contact does not
					// belong to any of
				...
				// Set nickname and store in memory only if subscription type is
				// not FROM.
				// Roster items with subscription type FROM that exist only
				// because of shared
				// groups will be recreated on demand in #getRosterItem(JID) and
				// #isRosterItem()
				// but will never be stored in memory nor in the database. This
				// is an important
				// optimization to reduce objects in memory and avoid loading
				// users in memory
				...
				if (item.getSubStatus() != RosterItem.SUB_FROM) {
					...
					// Cache information about shared contacts with subscription
					// status FROM
					implicitFrom.put(item.getJid().toBareJID(),
							item.getInvisibleSharedGroupsNames());
				}
			} catch (UserNotFoundException e) {
				Log.error("Groups (" + groups
						+ ") include non-existent username (" + jid.getNode()
						+ ")");
			}
		}
	...
	/**
	 * Returns true if the specified user is a member of the roster, false
	 * otherwise.
	 * 
	 * @param user
	 *            the user object to check.
	 * @return true if the specified user is a member of the roster, false
	 *         otherwise.
	 */
	public boolean isRosterItem(JID user) {
		// Optimization: Check if the contact has a FROM subscription due to
		// shared groups
		// (only when not present in the rosterItems collection)
		return rosterItems.containsKey(user.toBareJID())
				|| getImplicitRosterItem(user) != null;
	}
	 ...
	 * <p>
	 * 
	 * Note: Roster items with subscription type FROM that exist only because of
	 * shared groups are not going to be returned.
	 * 
	...
	/**
	 * Returns the roster item that is associated with the specified JID. If no
	 * roster item was found then a UserNotFoundException will be thrown.
	 * 
	 * @param user
	 *            the XMPPAddress for the roster item to retrieve
	 * @return The roster item associated with the user XMPPAddress.
	 * @throws UserNotFoundException
	 *             if no roster item was found for the specified JID.
	 */
	...
	/**
	 * Returns a roster item if the specified user has a subscription of type
	 * FROM to this user and the susbcription only exists due to some shared
	 * groups or otherwise <tt>null</tt>. This method assumes that this user
	 * does not have a subscription to the contact. In other words, this method
	 * will not check if there should be a subscription of type TO ot BOTH.
	 * 
	 * @param user
	 *            the contact to check if he is subscribed to the presence of
	 *            this user.
	 * @return a roster item if the specified user has a subscription of type
	 *         FROM to this user and the susbcription only exists due to some
	 *         shared groups or otherwise null.
	 */
	private RosterItem getImplicitRosterItem(JID user) {
		...
		if (invisibleSharedGroups != null) {
			RosterItem rosterItem = new RosterItem(user, RosterItem.SUB_FROM,
					RosterItem.ASK_NONE, RosterItem.RECV_NONE, "", null);
	 ...
	 * contain the same user address as an existing item.
	 * 
	 * @param user
	 *            The item to add to the roster.
	 * @param push
	 *            True if the new item must be pushed to the user.
	 * @param persistent
	 *            True if the new roster item should be persisted to the DB.
	 */
	public RosterItem createRosterItem(JID user, boolean push,
	 ...
	 *            The item to add to the roster.
	 * @param nickname
	 *            The nickname for the roster entry (can be null).
	 * @param push
	 *            True if the new item must be push to the user.
	 * @param persistent
	 *            True if the new roster item should be persisted to the DB.
	 * @param groups
	 *            The list of groups to assign this roster item to (can be null)
	 */
	public RosterItem createRosterItem(JID user, String nickname,
	...
	/**
	 * Create a new item to the roster based as a copy of the given item. Roster
	 * items may not be created that contain the same user address as an
	 * existing item in the roster.
	 * 
	 * @param item
	...
	public void createRosterItem(org.xmpp.packet.Roster.Item item)
			throws UserAlreadyExistsException, SharedGroupException {
		provideRosterItem(item.getJID(), item.getName(), new ArrayList<String>(
				item.getGroups()), true, true);
	}
	 ...
	 * @param persistent
	 *            True if the new roster item should be persisted to the DB.
	 * @return The newly created roster items ready to be stored by the Roster
	 *         item's hash table
	 */
	protected RosterItem provideRosterItem(JID user, String nickname,
			List<String> groups, boolean push, boolean persistent)
			throws UserAlreadyExistsException, SharedGroupException {
		if (groups != null && !groups.isEmpty()) {
			// Raise an error if the groups the item belongs to include a shared
			// group
			for (String groupDisplayName : groups) {
				Collection<Group> groupsWithProp = GroupManager.getInstance()
						.search("sharedRoster.displayName", groupDisplayName);
				Iterator<Group> itr = groupsWithProp.iterator();
				while (itr.hasNext()) {
					Group group = itr.next();
					String showInRoster = group.getProperties().get(
							"sharedRoster.showInRoster");
					if (showInRoster != null && !showInRoster.equals("nobody")) {
						throw new SharedGroupException(
								"Cannot add an item to a shared group");
					}
				}
			}
		}
		...
		persistent = RosterEventDispatcher.addingContact(this, rosterItem,
				persistent);
	 ...
	 * @throws UserNotFoundException
	 *             If the roster item for the given user doesn't already exist
	 */
	public void updateRosterItem(RosterItem item) throws UserNotFoundException {
		// Check if we need to convert an implicit roster item into an explicit
		// one
		...
		if (rosterItems.putIfAbsent(item.getJid().toBareJID(), item) == null && false) {
		...
		if (item.getID() == 0) {
			// Make the item persistent if a new nickname has been set for a
			// shared contact
			if (item.isShared()) {
				// Do nothing if item is only shared and it is using the default
				// user name
				if (item.isOnlyShared()) {
					String defaultContactName;
					try {
						defaultContactName = UserNameManager.getUserName(item
								.getJid());
					} catch (UserNotFoundException e) {
						// Cannot update a roster item for a local user that
						// does not exist
				...
				// Item is not persistent and it does not belong to a shared
				// contact so do nothing
			}
		} else {
			// Update the backend data store
			rosterItemProvider.updateItem(username, item);
		}
		currVersion = item.getCurrVersion();
		...
		/*
		 * if (item.getSubStatus() == RosterItem.SUB_BOTH || item.getSubStatus()
		 * == RosterItem.SUB_TO) { probePresence(item.getJid()); }
		 */
		// Fire event indicating that a roster item has been updated
		RosterEventDispatcher.contactUpdated(this, item);
	}
	 ...
	 * @param doChecking
	 *            flag that indicates if checkings should be done before
	 *            deleting the user.
	 * @return The roster item being removed or null if none existed
	 * @throws SharedGroupException
	 *             if the user to remove belongs to a shared group
	 */
	public RosterItem deleteRosterItem(JID user, boolean doChecking)
			throws SharedGroupException {
		// Answer an error if user (i.e. contact) to delete belongs to a shared
		// group
		RosterItem itemToRemove = rosterItems.get(user.toBareJID());
		if (doChecking && itemToRemove != null
				&& !itemToRemove.getSharedGroups().isEmpty()) {
			throw new SharedGroupException(
					"Cannot remove contact that belongs to a shared group");
		}

		if (itemToRemove != null) {
			RosterItem.SubType subType = itemToRemove.getSubStatus();

			// Cancel any existing presence subscription between the user and
			// the contact
			...
			// cancel any existing presence subscription between the contact and
			// the user
			if (subType == RosterItem.SUB_FROM
					|| subType == RosterItem.SUB_BOTH) {
				Presence presence = new Presence();
				presence.setFrom(server.createJID(username, null));
				presence.setTo(itemToRemove.getJid());
			...
			// If removing the user was successful, remove the user from the
			// subscriber list:
			RosterItem item = rosterItems.remove(user.toBareJID());

			if (item != null) {
				// Delete the item from the provider if the item is persistent.
				// RosteItems that only
				// belong to shared groups won't be persistent
				if (item.getID() > 0) {
					// If removing the user was successful, remove the user from
					// the backend store
					rosterItemProvider.deleteItem(username, item.getJid().getNode());
				}
			...
			if (item != null) {
				...
				// If the contact being removed is not a local user then ACK
				// unsubscription
	...
	/**
	 * <p>
	 * Obtain a 'roster reset', a snapshot of the full cached roster as an
	 * Roster.
	 * </p>
	 * 
	...
	public org.xmpp.packet.Roster getReset(long v) {
		org.xmpp.packet.Roster roster = new org.xmpp.packet.Roster();

		// Add the roster items (includes the personal roster and shared groups)
		// to the answer
		for (RosterItem item : rosterItems.values()) {
			// Do not include items with status FROM that exist only because of
			// shared groups
			if (item.getCurrVersion() > currVersion) {
				currVersion = item.getCurrVersion();
			}
			if (item.getCurrVersion() <= v
					|| (item.isOnlyShared() && item.getSubStatus() == RosterItem.SUB_FROM)) {
				continue;
			}

			org.xmpp.packet.Roster.Ask ask = getAskStatus(item.getAskStatus());
			org.xmpp.packet.Roster.Subscription sub = org.xmpp.packet.Roster.Subscription
					.valueOf(item.getSubStatus().getName());
			// Set the groups to broadcast (include personal and shared groups)
			List<String> groups = new ArrayList<String>(item.getGroups());
			if (groups.contains(null)) {
				Log.warn("A group is null in roster item: " + item.getJid()
						+ " of user: " + getUsername());
			}
			for (Group sharedGroup : item.getSharedGroups()) {
				String displayName = sharedGroup.getProperties().get(
						"sharedRoster.displayName");
				if (displayName != null) {
					groups.add(displayName);
				} else {
					// Do not add the shared group if it does not have a
					// displayName.
			...
			if (item.getSubStatus() != RosterItem.SUB_NONE
					|| item.getRecvStatus() != RosterItem.RECV_SUBSCRIBE) {
				roster.addItem(item.getJid(), item.getNickname(), ask, sub,
						groups);
			}
		}
		if (v != -1) {
			roster.getChildElement().addAttribute("ver",
					"" + getCurrVersion());
		}
	 ...
	 * <p/>
	 * <p>
	 * Any presence change typically results in a broadcast to the roster
	 * members.
	 * </p>
	 * 
	 * @param packet
	 *            The presence packet to broadcast
	 */
	public void broadcastPresence(Presence packet) {
		...
		if (from != null) {
			// Try to use the active list of the session. If none was found then
			// try to use
		...
		if (list == null) {
			// No privacy list was found (based on the session) so check if
			// there is a default list
			list = PrivacyListManager.getInstance().getDefaultPrivacyList(
					username);
		}
		...
		for (RosterItem item : rosterItems.values()) {
			if (item.getSubStatus() == RosterItem.SUB_BOTH
				...
				if (list != null && list.shouldBlockPacket(packet)) {
					// Outgoing presence notifications are blocked for this
					// contact
					continue;
				}
				JID searchNode = new JID(item.getJid().getNode(), item.getJid()
						.getDomain(), null, true);
				for (JID jid : routingTable.getRoutes(searchNode, null)) {
					try {
						routingTable.routePacket(jid, packet, false);
					} catch (Exception e) {
						// Theoretically only happens if session has been
						// closed.
		...
		// Broadcast presence to shared contacts whose subscription status is
		// FROM
	...
	/**
	 * Returns the list of users that belong ONLY to a shared group of this
	 * user. If the contact belongs to the personal roster and a shared group
	 * then it wont' be included in the answer.
	 * 
	 * @param sharedGroups
	 *            the shared groups of this user.
	 * @return the list of users that belong ONLY to a shared group of this
	 *         user.
	 */
	private Map<JID, List<Group>> getSharedUsers(Collection<Group> sharedGroups) {
		// Get the users to process from the shared groups. Users that belong to
		// different groups
		...
		for (Group group : sharedGroups) {
			...
			Collection<JID> users = rosterManager.getSharedUsersForRoster(
					group, this);
			// Add the users of the group to the general list of users to
			// process
			JID userJID = getUserJID();
			for (JID jid : users) {
				// Add the user to the answer if the user doesn't belong to the
				// personal roster
	...
	private void broadcast(org.xmpp.packet.Roster roster) {
		...
		roster.getChildElement().addAttribute("ver", ""+getCurrVersion());
		sessionManager.userBroadcast(username, roster);
	}

	/**
	 * Broadcasts the RosterItem to all the connected resources of this user.
	 * Due to performance optimizations and due to some clients errors that are
	 * showing items with subscription status FROM we added a flag that
	 * indicates if a roster items that exists only because of a shared group
	 * with subscription status FROM will not be sent.
	 * 
	 * @param item
	...
	public void broadcast(RosterItem item, boolean optimize) {
		// Do not broadcast items with status FROM that exist only because of
		// shared groups
		if (optimize && item.isOnlyShared()
				&& item.getSubStatus() == RosterItem.SUB_FROM) {
			return;
		}
		// Set the groups to broadcast (include personal and shared groups)
		...
		roster.addItem(item.getJid(), item.getNickname(), getAskStatus(item
				.getAskStatus()), org.xmpp.packet.Roster.Subscription
				.valueOf(item.getSubStatus().getName()), groups);
		broadcast(roster);
	}

	/**
	 * Sends a presence probe to the probee for each connected resource of this
	 * user.
	 */
	...
	public int getCachedSize() throws CannotCalculateSizeException {
		...
		// of the content of each field, if that content is likely to be
		// eligable for
		// garbage collection if the Roster instance is dereferenced.
		int size = 0;
		size += CacheSizes.sizeOfObject(); // overhead of object
		size += CacheSizes.sizeOfCollection(rosterItems.values()); // roster
																	// item
																	// cache
	...
	/**
	 * Update the roster since a group user has been added to a shared group.
	 * Create a new RosterItem if the there doesn't exist an item for the added
	 * user. The new RosterItem won't be saved to the backend store unless the
	 * user explicitly subscribes to the contact's presence, renames the contact
	 * in his roster or adds the item to a personal group. Otherwise the shared
	 * group will be added to the shared groups lists. In any case an update
	 * broadcast will be sent to all the users logged resources.
	 * 
	 * @param group
	 *            the shared group where the user was added.
	 * @param addedUser
	 *            the contact to update in the roster.
	 */
	void addSharedUser(Group group, JID addedUser) {
		...
		try {
			...
			try {
				...
				item = new RosterItem(addedUser, RosterItem.SUB_BOTH,
						RosterItem.ASK_NONE, RosterItem.RECV_NONE, nickname,
						null);
				...
				Log.error("Group (" + group.getName()
						+ ") includes non-existent username (" + addedUser
						+ ")");
			}
		}

		// If an item already exists then take note of the old subscription
		// status
		...
		Collection<Group> userGroups = GroupManager.getInstance().getGroups(
				getUserJID());
		...
		// Set subscription type to BOTH if the roster user belongs to a shared
		// group
		...
		if (rosterManager.hasMutualVisibility(getUsername(), userGroups,
				addedUser, sharedGroups)) {
			item.setSubStatus(RosterItem.SUB_BOTH);
		}
		// Update the subscription status depending on the group membership of
		// the new
		// user and this user
		...
		// If the item already exists then check if the subscription status
		// should be
		...
		if (item.isOnlyShared() && item.getSubStatus() == RosterItem.SUB_FROM) {
			...
			rosterItems.remove(item.getJid().toBareJID());
			// Cache information about shared contacts with subscription status
			// FROM
			implicitFrom.put(item.getJid().toBareJID(),
					item.getInvisibleSharedGroupsNames());
		} else {
	...
	/**
	 * Adds a new contact that belongs to a certain list of groups to the
	 * roster. Depending on the contact's groups and this user's groups, the
	 * presence subscription of the roster item may vary.
	 * 
	 * @param addedUser
	 *            the new contact to add to the roster
	 * @param groups
	 *            the groups where the contact is a member
	 */
	void addSharedUser(JID addedUser, Collection<Group> groups, Group addedGroup) {
		...
		if (rosterManager.hasMutualVisibility(getUsername(), userGroups,
			...
			// Add to the item the groups of this user that generated a FROM
			// subscription
			// Note: This FROM subscription is overridden by the BOTH
			// subscription but in
			// fact there is a TO-FROM relation between these two users that
			// ends up in a
			// BOTH subscription
			for (Group group : userGroups) {
				if (!group.isUser(addedUser)
						&& rosterManager.isGroupVisible(group, addedUser)) {
					// Add the shared group to the list of invisible shared
					// groups
					item.addInvisibleSharedGroup(group);
				}
			}
			...
			// Assume by default that the contact has subscribed from the
			// presence of
			// this user
			item.setSubStatus(RosterItem.SUB_FROM);
	...
	/**
	 * Update the roster since a group user has been deleted from a shared
	 * group. If the RosterItem (of the deleted contact) exists only because of
	 * of the sahred group then the RosterItem will be deleted physically from
	 * the backend store. Otherwise the shared group will be removed from the
	 * shared groups lists. In any case an update broadcast will be sent to all
	 * the users logged resources.
	 * 
	 * @param sharedGroup
	...
	void deleteSharedUser(Group sharedGroup, JID deletedUser) {
		try {
			...
			int groupSize = item.getSharedGroups().size()
					+ item.getInvisibleSharedGroups().size();
			if (item.isOnlyShared() && groupSize == 1) {
				// Do nothing if the existing shared group is not the
				// sharedGroup to remove
				if (!item.getSharedGroups().contains(sharedGroup)
						&& !item.getInvisibleSharedGroups().contains(
								sharedGroup)) {
					return;
				}
				// Delete the roster item from the roster since it exists only
				// because of this
				// group which is being removed
				deleteRosterItem(deletedUser, false);
			} else {
				// Remove the removed shared group from the list of shared
				// groups
				item.removeSharedGroup(sharedGroup);
				// Update the subscription of the item based on the remaining
				// groups
				if (item.isOnlyShared()) {
					Collection<Group> userGroups = GroupManager.getInstance()
							.getGroups(getUserJID());
					Collection<Group> sharedGroups = new ArrayList<Group>();
					sharedGroups.addAll(item.getSharedGroups());
					...
					// that is mutually visible with a shared group of the new
					// roster item
					if (rosterManager.hasMutualVisibility(getUsername(),
							userGroups, deletedUser, sharedGroups)) {
						item.setSubStatus(RosterItem.SUB_BOTH);
			...
			// Do nothing. Checkings are disabled so this exception should never
			// happen.
		} catch (UserNotFoundException e) {
			// Do nothing since the contact does not exist in the user's roster.
			// (strange case!)
		}
	}
	...
	void deleteSharedUser(JID deletedUser, Group deletedGroup) {
		try {
			...
			int groupSize = item.getSharedGroups().size()
					+ item.getInvisibleSharedGroups().size();
			if (item.isOnlyShared() && groupSize == 1 &&
			// Do not delete the item if deletedUser belongs to a public group
			// since the
			// subcription status will change
					!(deletedGroup.isUser(deletedUser) && RosterManager
							.isPublicSharedGroup(deletedGroup))) {
				// Delete the roster item from the roster since it exists only
				// because of this
				// group which is being removed
				deleteRosterItem(deletedUser, false);
			} else {
				// Remove the shared group from the item if deletedUser does not
				// belong to a
				// public group
				if (!(deletedGroup.isUser(deletedUser) && RosterManager
						.isPublicSharedGroup(deletedGroup))) {
					item.removeSharedGroup(deletedGroup);
				}
				// Get the groups of the deleted user
				Collection<Group> groups = GroupManager.getInstance()
						.getGroups(deletedUser);
				// Remove all invalid shared groups from the roster item
				for (Group group : groups) {
					if (!rosterManager.isGroupVisible(group, getUserJID())) {
						// Remove the shared group from the list of shared
						// groups
						item.removeSharedGroup(group);
					}
				}

				// Update the subscription of the item **based on the item
				// groups**
				if (item.isOnlyShared()) {
					Collection<Group> userGroups = GroupManager.getInstance()
							.getGroups(getUserJID());
					...
					// roster item
					if (rosterManager.hasMutualVisibility(getUsername(),
							userGroups, deletedUser, groups)) {
						item.setSubStatus(RosterItem.SUB_BOTH);
					} else {
						// Assume by default that the contact has subscribed
						// from the presence of
						// this user
						item.setSubStatus(RosterItem.SUB_FROM);
						// Check if the user may see the new contact in a shared
						// group
						for (Group group : groups) {
	...
	/**
	 * A shared group of the user has been renamed. Update the existing roster
	 * items with the new name of the shared group and make a roster push for
	...
	void shareGroupRenamed(Collection<JID> users) {
		...
		for (JID user : users) {
			...
			try {
				...
				// Do nothing since the contact does not exist in the user's
				// roster. (strange case!)
			}
		}
	}
	...
	public void writeExternal(ObjectOutput out) throws IOException {
		...
		ExternalizableUtil.getInstance().writeExternalizableMap(out,
				rosterItems);
	...
	public void readExternal(ObjectInput in) throws IOException,
		...
		ExternalizableUtil.getInstance().readExternalizableMap(in, rosterItems,
