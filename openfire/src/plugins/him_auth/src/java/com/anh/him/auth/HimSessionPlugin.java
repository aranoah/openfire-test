...
public class HimSessionPlugin implements Plugin ,PropertyEventListener{
	...
	private void register() {
		String serverAddress = XMPPServer.getInstance().getServerInfo().getXMPPDomain();
	    final  MessageRouter router = XMPPServer.getInstance().getMessageRouter();
		final Message message = new Message();
        message.setTo("admin@"+serverAddress);
        message.setFrom(serverAddress);
        message.setSubject("Attachment");
        message.setBody("Hello Dear");
        ...
        TaskEngine.getInstance().schedule(messageTask, 100);
