 ...
 * {@link java.util.concurrent.ExecutorService} and {@link java.util.Timer}. Any
 * {@link java.util.TimerTask} that's scheduled to be run in the future will
 * automatically be run using the thread executor's thread pool. This means that
 * the standard restriction that TimerTasks should run quickly does not apply.
 * 
 * @author aniyus
 */
public class TaskManager {
	 ...
	 */
	private TaskManager() {
		timer = new Timer("timer-him", false);
		executor = Executors.newCachedThreadPool(new ThreadFactory() {
			...
			public Thread newThread(Runnable runnable) {
				...
				Thread thread = new Thread(Thread.currentThread()
						.getThreadGroup(), runnable, "pool-him"
	...
	public void cancelScheduledTask(TimerTask task) {
		TaskManager.TimerTaskWrapper taskWrapper = wrappedTasks.remove(task);
		if (taskWrapper != null) {
			taskWrapper.cancel();
		}
	}
