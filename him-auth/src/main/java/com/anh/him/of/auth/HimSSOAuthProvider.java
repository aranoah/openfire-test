...
public class HimSSOAuthProvider implements AuthProvider {
	...
	private static Logger log = LoggerFactory.getLogger(HimSSOAuthProvider.class);
	private String authSecurityKey =JiveProperties.getInstance().getProperty("him.auth.secret-key","default-him-key");
	/**
	...
	/**
	 * @param authSecurityKey the authSecurityKey to set
	 */
	...
	if(password==null || password.length()==0)
			throw new UnauthorizedException("Invalid session, please relogin to auth server");
	    String[] tokens = password.split(":");
	    if(tokens.length!=3){
	    	throw  new UnauthorizedException("Invalid session, please relogin to auth server");
	    }
	    //0-md5(sessionid:timestamp:password)  1: server token
	    authSecurityKey =JiveProperties.getInstance().getProperty("him.auth.secret-key","default-him-key");
	    String sessionPasswd = StringUtils.hash( username+":"+tokens[0]+":"+tokens[1]+":"+authSecurityKey,"MD5");
